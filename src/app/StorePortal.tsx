...
export function StoreLoginPage({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // ── MongoDB API login — backend handles store isolation ──────────────────
    try {
      ...
      const r = await fetch(`${BASE}/auth/login`, {
        ...
        body: JSON.stringify({
          ...
          password,
          storeSlug: store.slug, // backend filters by this store only
        }),
        signal: AbortSignal.timeout(8000),
      });
      const d = await r.json();
      if (r.ok && d.token && d.user) {
        // Backend already verified the user belongs to this store
        ...
        onLogin({
          ...
          password: "", storeSlug: d.user.storeSlug || store.slug,
        });
        return;
      }
      if (!r.ok && d.message) {
        // Real error from backend (wrong password, suspended, etc.)
        setLoading(false);
        setError(d.message);
        return;
      }
      ...
      setLoading(false);
      setError("تعذر الاتصال بالخادم — تأكد من الاتصال بالإنترنت");
      return;
    }
    setLoading(false);
